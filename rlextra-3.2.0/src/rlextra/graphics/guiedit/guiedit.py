...
import traceback
from reportlab.lib.utils import rl_exec, asNative, isNative, isNonPrimitiveInstance
...
Pmw.aboutcopyright('Copyright ReportLab Inc. 2001-2010\nAll rights reserved')
...
_ifmainPat = re.compile('^if\\s+(__name__\\s*==\\s*([\'\"])__main__[\"\']|([\'\"])__main__[\'\"]\\s*==\\s*__name__)\s*:')
...
class GUIEditor:
	...
	def getSampleAttr(self,name):
		...
		return eval('self.project.sample'+name)
