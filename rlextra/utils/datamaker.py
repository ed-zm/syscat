#copyright ReportLab europe Limited 2001-2012
...
class DataMaker:
    ...
    def fetchNextDataSet(self):
        ...
        if len(self._data) == 0:
            ...
            block = []
            data = self._data
            grpCol = getattr(self,'_groupingCol',self.groupingColumn)
            row = 0
            initialGroup = data[row][grpCol]
            while 1:
                block.append(data[0])
                data = data[1:]
                if len(data) == 0:
                    break
                if data[0][grpCol] != initialGroup:
                    break
            self._data = data
            return block
    ...
    def connect(self,verbose=0):
        ...
        if n>0:
            ...
            if j is not None:
                ...
                self._names = names = list(map(str.strip,list(map(str.lower,_processLine(headers[j],self.sep)))))
        else:
            names = getattr(self,'columnNames',None)
            if names: self._names = names = list(map(str.strip,list(map(str.lower,names))))

        sql = self.sql
        if isinstance(sql, collections.Callable): sql = sql()
        sql.strip()
        self._groupingCol = self.groupingColumn
        if sql:
            if not names: raise ValueError("sql attribute and no column names")
            try:
                self._checkCSVColumnNames('groupingColumn',names)
                self._groupingCol = names.index(self.groupingColumn)
            except:
                pass
    ...
    def _checkCSVColumnNames(self,aName,names):
        ...
        for i in range(len(A)):
            ...
            if not isinstance(a,str):
                raise ValueError("%s instance has sql, but attribute %s contains non-string '%s'" % (self.__class__.__name__,aName,str(a)))
            A[i] = a = a.lower()
            if a not in names:
                raise ValueError("%s instance has sql, but attribute %s contains '%s' not in column names" % (self.__class__.__name__,aName,a))
        if fnc: A = fnc(A)
        else: A = A[0]
